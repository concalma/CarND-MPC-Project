...
// reference velocity for cost calculation. From Lesson 19.3
double ref_v = 100;
 ...
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    for (int t = 1; t < N; t++) {
      ...
      fg[1 + psi_start + t] = psi1 - (psi0 - /*reversing for sim*/ v0 * delta0 / Lf * dt);
      fg[1 + v_start + t] = v1 - (v0 + a0 * dt);
      fg[1 + cte_start + t] =  cte1 - ((f0 - y0) + (v0 * CppAD::sin(epsi0) * dt));
      fg[1 + epsi_start + t] = epsi1 - ((psi0 - psides0) - /*reversing for sim*/  v0 * delta0 / Lf * dt);
    }
  }
};
...
// RETURNS
// next state. x+1  0:5
// delta,a          6:7   -> steering and throttle
